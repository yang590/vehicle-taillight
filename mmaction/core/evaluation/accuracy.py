...
#custom
def cls_accuracy(scores, labels):

    scores = np.where(scores>0.5, 1, 0)

    accs = []
    for cls in range(labels.shape[-1]):
        cls_acc = np.equal(scores[:, cls], labels[:, cls]).sum() / labels[:, cls].shape[0]
        accs.append(cls_acc)

    return accs

def mean_cls_accuracy(scores, labels):
    scores = np.where(scores > 0.3, 1, 0)

    acc = np.equal(scores, labels).sum() / labels.size
    return acc

#custom
def cls_precision_recall(scores, labels):
    scores = np.where(scores > 0.5, 1, 0)

    # res = metrics.classification_report(labels, scores, target_names=['class1', 'class2', 'class3', 'class4'])
    # return res

    # prfs = []
    # for i in range(labels.shape[1]):
    #     pi = metrics.precision_score(labels[:, i], scores[:, i], average='macro')
    #     ri = metrics.recall_score(labels[:, i], scores[:, i], average='macro')
    #     fi = metrics.f1_score(labels[:, i], scores[:, i], average='macro')
    #     prfs.append([pi, ri, fi])
    # p = sum([prfs[i][0] for i in range(4)])/4
    # r = sum([prfs[i][1] for i in range(4)])/4
    # f = sum([prfs[i][2] for i in range(4)])/4
    # prfs = [[p, r, f]]

    # p = metrics.precision_score(labels, scores, average='macro')
    # r = metrics.recall_score(labels, scores, average='macro')
    # f = metrics.f1_score(labels, scores, average='macro')
    # prfs = [[p, r, f]]
    # return prfs


#custom
def aprf_custom(scores, labels):
    scores = np.where(scores > 0.3, 1, 0)

    mcm = metrics.multilabel_confusion_matrix(labels, scores)
    ...
    for i in range(mcm.shape[0]):
        ...
        a_i = round((tp_i+tn_i)/(tp_i+fp_i+tn_i+fn_i), 4)
        p_i = round(tp_i/(tp_i+fp_i), 4)
        r_i = round(tp_i/(tp_i+fn_i), 4)
        f_i = round((2*p_i*r_i)/(p_i+r_i), 4)
        prfs.append([a_i, p_i, r_i, f_i])
    ...
    ala0 = round(sum([x[0] for x in prfs])/4, 4)
    alp0 = round(sum([x[1] for x in prfs])/4, 4)
    alr0 = round(sum([x[2] for x in prfs])/4, 4)
    alf0 = round(sum([x[3] for x in prfs])/4, 4)
    prfs.insert(0, [ala0, alp0, alr0, alf0])

    ala = round((alm[0]+alm[2])/(alm[0]+alm[1]+alm[2]+alm[3]), 4)
    alp = round(alm[0]/(alm[0]+alm[1]), 4)
    alr = round(alm[0]/(alm[0]+alm[3]), 4)
    alf = round((2*alp*alr)/(alp+alr), 4)
    prfs.insert(0, [ala, alp, alr, alf])
