...
class Flip:
    ...
    def __call__(self, results):
        ...
        if self.flip_label_map is not None and flip:
            ...
            #                                            results['label'])
            #custom
            lv, rv = results['label'][2], results['label'][3]
            results['label'][2] = rv
            results['label'][3] = lv
...
class Rotation:
    def __init__(self,
        ...
        self.rotaion_ratio = rotaion_ratio
        self.angle = angle
        self.scale = scale

    def __call__(self, results):
        rotation = np.random.rand() < self.rotaion_ratio
        ...
        if rotation:
            ...
            M = cv2.getRotationMatrix2D(center, ang, self.scale)

            imgs = []
            for i, img in enumerate(results['imgs']):
                imgs.append(cv2.warpAffine(src=img, M=M, dsize=(h, w)))
            results['imgs'] = imgs
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    f'rotaion_ratio={self.rotaion_ratio})')
        return repr_str
...
class GammaTrans:
    ...
    def __call__(self, results):
        gamma = np.random.rand() < self.gamma_ratio
        results['gamma'] = gamma

        if gamma:
            sca = random.uniform(1-self.scale, 1+self.scale)
            imgs = []
            for i, img in enumerate(results['imgs']):
                imgs.append(exposure.adjust_gamma(img, sca))
            results['imgs'] = imgs
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    f'gamma_ratio={self.gamma_ratio})')
        return repr_str
